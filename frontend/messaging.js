...
function renderMsgButtons(type, record, phoneKey, emailKey) {
  ...
  const waBtn = `<button class="btn-edit btn-icon-action" title="WhatsApp" onclick="showComposeModal(window.__msgBtnCache[${idx}])" style="background:rgba(37,211,102,0.12);border-color:rgba(37,211,102,0.25);min-width:26px;">${waIcon}</button>`;
  const emailBtn = `<button class="btn-edit btn-icon-action" title="Email" onclick="(function(){var o=window.__msgBtnCache[${idx}];showComposeModal({type:o.type,record:o.record,phone:o.phone,email:o.email,startTab:'email'});})()" style="background:rgba(99,102,241,0.10);border-color:rgba(99,102,241,0.20);min-width:26px;">${emailIcon}</button>`;
  return waBtn + emailBtn;
}
