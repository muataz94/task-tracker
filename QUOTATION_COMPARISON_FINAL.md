# Task Tracker — Quotation Comparison Tab
# Claude Code: Read ALL files in E:\task-tracker\ before any changes.
# List every file you read. Implement step by step. List every file modified after.

---

## EXCEL TEMPLATE REFERENCE (BCS_PR_SW3104.xls analyzed)

The Excel has 5 sections:
1. Header: PR#, Description, Dept, Dates, Total Value, Currency, Delivery Term, Warranty Term
2. Vendors Data Table: Vendor Name, Total Cost, Spec Compliance%, Install Compliance%, Delivery Days, Warranty Months, Payment Compliance%, Commitment%
3. Scores Table: 6 weighted scores auto-calculated per vendor (Price=40, Req=30, Delivery=10, Warranty=10, Payment=5, Commitment=5)
4. Winning Bid: Winner name, Total Score, Total Amount, Comment/Recommendation
5. Committee Signatures: Head of Committee, Requester, Requester Management, Supply Chain Officer, Head of Supply Chain

---

## STEP 0 — GOOGLE SHEETS (tell user BEFORE running)

Create TWO new tabs in Google Sheets:

**Tab: `Comparisons`** — paste as row 1 headers:
```
id | pr_number | request_description | requesting_dept | request_date | awarding_date | total_pr_value | currency | delivery_term_days | warranty_term_months | w_price | w_requirements | w_delivery | w_warranty | w_payment | w_commitment | winner_vendor | winner_score | winner_amount | winner_comment | status | linked_po_id | head_of_committee | requester_name | requester_management | supply_chain_officer | head_of_supply_chain | created_at | created_by | updated_at
```

**Tab: `ComparisonVendors`** — paste as row 1 headers:
```
id | comparison_id | vendor_name | annex_ref | total_cost | spec_compliance | install_compliance | delivery_days | warranty_months | payment_compliance | commitment_pct | price_score | requirements_score | delivery_score | warranty_score | payment_score | commitment_score | total_score | rank | created_at
```

Also add column `comparison_id` to the rightmost column of the **POs tab**.
Freeze row 1 on all new tabs: View → Freeze → 1 row.

---

## STEP 1 — UPDATE backend/Code.gs

Add to the switch(action) block:
```javascript
...
function updateComparison(id, data, vendors, updatedBy) {
  ...
  // Delete old vendor rows and re-insert
  const vs = getSpreadsheet().getSheetByName('ComparisonVendors');
...
Redeploy Code.gs as **New version** after this change.

---

## STEP 2 — CREATE frontend/quotations.js (new file)

Create `E:\task-tracker\frontend\quotations.js` with this complete code:
...
function colColor(values, idx) {
  // Returns 'best' | 'worst' | 'mid' | 'neutral'
  ...
  if (Math.abs(maxV - minV) < 0.001) return 'neutral'; // all same
...
### Sidebar nav item (add after POs nav item):
```html
<li class="nav-item" data-view="quotations">
  <svg class="nav-svg" width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8">
  ...
  <span data-i18n="quotations">Quotations</span>
</li>
```

### View HTML (add before closing `</main>`):
```html
<div id="view-quotations" class="view hidden">
  <div class="tab-header glass" style="border-radius:var(--r-md);padding:1rem 1.25rem;margin-bottom:1rem;">
    <div class="tab-header-left">
      <div class="tab-header-icon" style="background:rgba(245,158,11,0.15);color:#fbbf24;">
      ...
      <div>
        <h2 data-i18n="quotations">Quotation Comparisons</h2>
        <p class="tab-subtitle" data-i18n="quotations_sub">Final Bids Comparison Table / جدول مقارنة العطاءات</p>
      </div>
    </div>
    <div class="tab-header-right">
      <button class="btn-add" onclick="showNewCompForm()">+ New Comparison</button>
    </div>
  </div>
  <div id="qc-wrap"></div>
</div>
...
### Script tag (add before inline script):
```html
<script src="quotations.js"></script>
```

### Confirm these CDNs are in `<head>`:
```html
...
## STEP 4 — UPDATE navigateTo() IN inline script

Add to navigateTo():
```javascript
else if (view === 'quotations') loadQuotations();
```
...
## STEP 5 — ADD TO i18n.js TRANSLATIONS
...
## STEP 6 — ADD CSS TO style.css

```css
/* ── QUOTATION COMPARISON ── */
.qc-form    { display:flex; flex-direction:column; gap:14px; }
.qc-sec     { border-radius:var(--r-md); padding:1.25rem 1.5rem; }
.qc-sec-hd  { display:flex; align-items:center; gap:8px; font-size:13px; font-weight:700; color:var(--text-1); letter-spacing:-.02em; margin-bottom:1rem; }
.qc-grid    { display:grid; grid-template-columns:repeat(auto-fit,minmax(195px,1fr)); gap:12px; }
.qc-wgrid   { display:grid; grid-template-columns:repeat(auto-fit,minmax(130px,1fr)); gap:10px; }
.qc-winput  { background:var(--glass-bg); border:1px solid var(--border); color:var(--text-1); padding:8px; border-radius:var(--r-sm); font-size:15px; font-weight:700; outline:none; text-align:center; font-family:'Inter',sans-serif; transition:border-color .15s; width:100%; }
.qc-winput:focus { border-color:var(--accent); box-shadow:0 0 0 2px rgba(167,139,250,.12); }
.qc-wtotal  { margin-left:auto; font-size:12px; font-weight:600; padding:3px 10px; border-radius:20px; transition:all .2s; }
.qc-vrow    { border-radius:var(--r-md); padding:1rem 1.25rem; margin-bottom:10px; }
.qc-vrow-hd { display:flex; align-items:center; justify-content:space-between; margin-bottom:10px; }
.qc-vnum    { font-size:12px; font-weight:700; color:var(--accent); background:rgba(167,139,250,.12); padding:3px 10px; border-radius:20px; }
.qc-vgrid   { display:grid; grid-template-columns:repeat(auto-fit,minmax(148px,1fr)); gap:10px; }
.qc-stbl    { width:100%; border-collapse:collapse; font-size:12px; }
.qc-stbl thead th { padding:8px 12px; background:var(--glass-bg); border-bottom:1px solid var(--border); font-size:10px; font-weight:600; color:var(--text-3); text-transform:uppercase; letter-spacing:.05em; text-align:right; }
.qc-stbl thead th:first-child { text-align:left; }
.qc-stbl tbody td { padding:10px 12px; border-bottom:1px solid var(--border); color:var(--text-2); text-align:right; }
.qc-stbl tbody td:first-child { text-align:left; }
.qc-stbl tbody tr:last-child td { border-bottom:none; }
.qc-stbl tbody tr:hover td { background:var(--glass-bg); }
.qc-winner-tr td { background:rgba(16,185,129,.05) !important; }
.qc-stbl tfoot td { padding:6px 12px; font-size:10px; color:var(--text-4); border-top:1px solid var(--border); }
.qc-actions { display:flex; gap:8px; justify-content:flex-end; flex-wrap:wrap; padding-bottom:2rem; }
.qc-list    { display:flex; flex-direction:column; gap:10px; }
.qc-card    { border-radius:var(--r-md); padding:1.1rem 1.25rem; cursor:pointer; transition:transform .2s ease,box-shadow .2s ease; }
.qc-card:hover { transform:translateY(-2px); box-shadow:var(--glass-shadow-lg); }
.qc-card-top    { display:flex; align-items:flex-start; justify-content:space-between; gap:10px; margin-bottom:8px; }
.qc-pr-badge    { font-size:10px; font-weight:600; color:var(--accent); background:rgba(167,139,250,.12); padding:2px 8px; border-radius:20px; display:inline-block; margin-bottom:4px; }
.qc-card-title  { font-size:14px; font-weight:600; color:var(--text-1); }
.qc-card-meta   { font-size:12px; color:var(--text-3); margin-top:2px; }
.qc-card-bottom { display:flex; align-items:center; gap:12px; flex-wrap:wrap; border-top:1px solid var(--border); padding-top:10px; margin-top:8px; }

/* Extra badge types */
.badge-in_review { background:rgba(245,158,11,.15); color:#fbbf24; border:1px solid rgba(245,158,11,.25); }
.badge-awarded   { background:rgba(16,185,129,.15); color:#34d399; border:1px solid rgba(16,185,129,.25); }
```

---

## STEP 7 — ADD Comparisons TO SHEET_FIELDS IN tables.js

Add this entry to SHEET_FIELDS for the detail panel to work:
```javascript
Comparisons: [
...
1. Read ALL files in E:\task-tracker\ before starting
2. Create `frontend/quotations.js` as a new file (Step 2)
3. Do NOT modify config.js or api.js content
4. Script load order: i18n.js → config.js → cache.js → api.js → tables.js → dashboard.js → kanban.js → chat.js → quotations.js → inline script
5. After Code.gs changes — tell user to REDEPLOY as new version
6. After all changes list every file modified or created
7. Remind user:
   - Create Comparisons tab in Google Sheets with headers from Step 0
   - Create ComparisonVendors tab in Google Sheets with headers from Step 0
   - Add comparison_id column to POs tab
   - Redeploy Apps Script Code.gs as new version
   - Run: git add . && git commit -m "quotation comparison tab" && git push
